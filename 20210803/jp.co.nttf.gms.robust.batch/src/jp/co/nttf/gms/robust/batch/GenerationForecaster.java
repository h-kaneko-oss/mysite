...
public final class GenerationForecaster extends CommonBatch {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info(MessageUtil.getMessage(MessageID.IROB067));
        }
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info(MessageUtil.getMessage(MessageID.IROB068));
        }
    }
    ...
    private void execute(String argDate) throws Exception {
        ...
        for (TDemandGenerationLocation dgLocation : listDGLocation) {
            ...
            int measurepointId = listGenerationMeasurepointId.getGenerationMeasurepointId()[AppConstants.BT_GENERATION_DATA_ORDER];
    ...
    private Process startDemandBatch() throws Exception {
        ...
        // 需要予測算出バッチファイルのパス
        String scriptPath = gmsConfig.getString(AppConstants.PROPERTY_ROBUST_DEMAND_FORECAST_SCRIPT);
        if (LOGGER.isInfoEnabled()) {
    ...
    /**
     * 抽出データテーブルに取得対象のデータが存在しているか確認を行うメソッド. <br>
     * <b>【注意】</b> 特になし.
     *
     * @param execDate 処理日時
     * @param demandMeasurepointId 需要計測ポイントID
     * @return 抽出データテーブルのデータ有無(存在する: true、存在しない: false)
     * @throws Exception 処理中に問題が発生した場合
     */
    private boolean isExistExtractvalue(String execDate, Integer generationMeasurepointId) throws Exception {
        ...
        param.setMeasuretime(new Timestamp(sdf.parse(execDate).getTime() - AppConstants.ROBUST_48HR_IN_MILLISEC));
    ...
    private List<TExtractvalue> getGenerationExtractData(String execDate, Integer generationMeasurepointId) throws Exception {
        ...
        try {
            ...
            // SQL実行
            TExtractvalueMapper mapper = session.getMapper(TExtractvalueMapper.class);
            list = mapper.getRangeDataMin00(generationMeasurepointId, startTS, endTS, 2);
        } finally {
            // セッションをクローズ
            if (null != session) {
                session.close();
        ...
        //for (int i=0; i<48; i++) {
            //System.out.println(list.get(i).getMeasuretime().toString());
        //}
    ...
    private List<TWeatherForecastG> getWeatherForecastG(String execDate, Integer locationId) throws Exception {
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info(MessageUtil.getMessage(MessageID.IROB079, sdf.format(startTS)));
            LOGGER.info(MessageUtil.getMessage(MessageID.IROB080, sdf.format(endTS)));
        }
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info(MessageUtil.getMessage(MessageID.IROB081, list.size()));
            LOGGER.info(MessageUtil.getMessage(MessageID.IROB082, sdf.format(list.get(0).getForecastTime())));
            LOGGER.info(MessageUtil.getMessage(MessageID.IROB083, sdf.format(list.get(list.size()-1).getForecastTime())));
        }
