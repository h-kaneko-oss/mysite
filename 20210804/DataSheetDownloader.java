    ...
    private void execute(int dgLocationId, String startDate, String endDate) throws Exception {
        ...
        while (curTime <= endTime) {
            ...
            if (listDForecast.size() == arraySumDMeasurevalues.length && listGForecast.size() == listSumGMeasurevalues.size()) {
                ...
                LOGGER.error(MessageUtil.getMessage(MessageID.EROB004, arraySumDMeasurevalues.length, listDForecast.size()));
                LOGGER.error(MessageUtil.getMessage(MessageID.EROB004, listSumGMeasurevalues.size(), listGForecast.size()));
                throw(new Exception());
            }
