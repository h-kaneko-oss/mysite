...
public final class SimulationCalculator extends CommonBatch {
    ...
    private void execute(String argDate) throws Exception {
        ...
        for (TDemandGenerationLocation dgLocation : listDGLocation) {
            ...
            TExtractvalue datasetSocMin = this.getGenerationExtractData(listGenerationMeasurepointId.getGenerationMeasurepointId()[AppConstants.G_SOC_MIN_ORDER]);
            socmin = datasetSocMin.getMeasurevalue();

            // t_extractvalueテーブルから最新のSoC下限設定(自立運転用)を取得
            TExtractvalue datasetSocIsland = this.getGenerationExtractData(listGenerationMeasurepointId.getGenerationMeasurepointId()[AppConstants.G_SOC_ISLAND_ORDER]);
            socisland = datasetSocIsland.getMeasurevalue();

            // t_extractvalueテーブルから最新の双方向電源動作ステータスを取得
            TExtractvalue datasetBatStatus = this.getGenerationExtractData(listGenerationMeasurepointId.getGenerationMeasurepointId()[AppConstants.G_STATUS_DATA_ORDER]);
            ...
            TExtractvalue datasetSoc = this.getGenerationExtractData(listGenerationMeasurepointId.getGenerationMeasurepointId()[AppConstants.G_SOC_DATA_ORDER]);
    ...
    private List<TDemandForecast> getDemandForecast(Integer dgLocationId, String execDate) throws Exception {
        ...
        long startTime  = sdf.parse(execDate).getTime();
        ...
        try {
            ...
            for (int i=0; i<48; i++) {
                Timestamp targetTS = new Timestamp(startTime + (i+1) * AppConstants.ROBUST_30MIN_IN_MILLISEC);
